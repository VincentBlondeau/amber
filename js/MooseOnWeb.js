...
$6;
$7=_st(cont)._a();
_st($7)._href_("#searchModal");
_st($7)._at_put_("role","button");
_st($7)._at_put_("data-toggle","modal");
_st($7)._class_("pull-right btn btn-mini btn-info");
_st($7)._with_("Search");
$8=_st($7)._onClick_((function(){
return smalltalk.withContext(function($ctx4) {return _st((smalltalk.MWSearch || MWSearch))._new();
}, function($ctx4) {$ctx4.fillBlock({},$ctx1)})}));
return $8;
...
args: ["html"],
source: "renderOn: html\x0a\x09html div \x0a    \x09class: self cssClass; \x0a        at: 'row' put: self number;\x0a    \x09with: [ :element | \x0a        \x09element div \x0a                with: [ :cont |\x0a                            cont span \x0a                            \x09    class:' label label-info';\x0a                            \x09\x09with: self content title.\x0a                            cont button \x0a                                class:'pull-right btn btn-mini btn-danger';\x0a                                type:'button';\x0a                                onClick: [ self close ];\x0a                                with: 'X'.\x0a                            cont a\x0a                                href:'#searchModal';\x0a                                at: 'role' put:'button';\x0a                                at: 'data-toggle' put:'modal';\x0a                                class:'pull-right btn btn-mini btn-info';\x0a                                with: 'Search';\x0a                                onClick: [ MWSearch new].\x0a            \x09]   .\x0a            element br.    \x0a            element with: self content.\x0a        ]\x0a\x09\x09",
messageSends: ["class:", "cssClass", "div", "at:put:", "number", "with:", "span", "title", "content", "button", "type:", "onClick:", "close", "href:", "a", "new", "br"],
...
smalltalk.addClass('MWSearch', smalltalk.Widget, [], 'MooseOnWeb');
smalltalk.addMethod(
"_cssId",
smalltalk.method({
selector: "cssId",
...
_st($1)._href_("#");
_st($1)._with_("Action");
$2=_st($1)._with_(_st(_st(content)._span())._class_("caret"));
$3=_st(content)._ul();
_st($3)._class_("dropdown-menu");
$4=_st($3)._with_((function(){
return smalltalk.withContext(function($ctx2) {return _st(_st(content)._li())._with_("Other action");
}, function($ctx2) {$ctx2.fillBlock({},$ctx1)})}));
return self}, function($ctx1) {$ctx1.fill(self,"renderActionListOn:",{content:content}, smalltalk.MWSearch)})},
args: ["content"],
source: "renderActionListOn: content \x0a\x09content a class: 'btn dropdown-toggle';\x0a                    \x09at: 'data-toggle' put:'dropdown';\x0a                        href:'#';\x0a                        with: 'Action';\x0a                        with: (content span class: 'caret').\x0a                    content ul class: 'dropdown-menu';\x0a                    \x09with: [content li  with: 'Other action'].\x0a\x0a       \x0a\x0a    \x0a",
messageSends: ["class:", "a", "at:put:", "href:", "with:", "span", "ul", "li"],
referencedClasses: []
}),
smalltalk.MWSearch);
...
args: ["html"],
source: "renderBodyOn: html\x0a\x09\x09html div class:' modal-body'; at:'max-height' put: '200'; with: [ :el |\x0a         \x09el div class: 'btn-group';\x0a            \x09with: [ :content | \x22self renderActionListOn: content \x22]\x0a        ]\x0a       \x0a\x0a    \x0a",
messageSends: ["class:", "div", "at:put:", "with:"],
referencedClasses: []
}),
smalltalk.MWSearch);
...
$6=_st($5)._with_("Save changes");
return $6;
}, function($ctx2) {$ctx2.fillBlock({el:el},$ctx1)})}));
return self}, function($ctx1) {$ctx1.fill(self,"renderFooterOn:",{html:html}, smalltalk.MWSearch)})},
args: ["html"],
source: "renderFooterOn: html\x0a\x09html div class:'modal-footer';\x0a    \x09with: [ :el |\x0a        \x09el button class:'btn'; at: 'data-dismiss' put:'modal'; at: 'aria-hidden' put:'true'; with:'Close'.\x0a\x09\x09\x09el button class:'btn btn-primary'; onClick: [ self search ]; with: 'Save changes'.\x0a        ]\x0a\x0a",
...
return self}, function($ctx1) {$ctx1.fill(self,"search",{}, smalltalk.MWSearch)})},
args: [],
source: "search\x0a\x09MWAnnouncer current announce: (\x0a    \x09MWSuccess new \x0a    ).\x0a    (self cssId asJQuery) modal: 'toggle'.",
messageSends: ["announce:", "new", "current", "modal:", "asJQuery", "cssId"],
referencedClasses: ["MWSuccess", "MWAnnouncer"]
}),
smalltalk.MWSearch);
